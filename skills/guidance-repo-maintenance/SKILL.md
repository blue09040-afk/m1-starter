...
- Use this skill for the guidance content change itself. After the change is confirmed, use `guidance-sync` for GitHub/local/installed-skill synchronization or for publishing those confirmed changes through a branch and PR.
- Read `skills/guidance-sync/references/repository_sync_workflow.md` for the detailed sync procedure. The former path `references/repository_sync_workflow.md` remains as a compatibility pointer.
