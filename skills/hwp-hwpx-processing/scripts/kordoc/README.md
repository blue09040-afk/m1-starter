...
직접 의존성은 `package.json`에서 exact pin합니다. Starter의 첫 검증 실행은 lockfile을 전제로 하지 않으며 workflow가 실행 중 lockfile을 생성해 artifact에 포함합니다. 템플릿으로 복제한 독립 저장소는 자신의 첫 성공 실행에서 runtime artifact를 새로 생성합니다.
